...
public class TTT_Model {
     ...
     * @return true if each entry of the specified row contains the specified player
     */
     ...
     * @param player the player to be looked for
     * @return true if each entry of the specified column contains the specified player
     */
    ...
    private boolean checkMinorDiagonal(TTT_Constants player) {
        for (int counter = 0; counter < board.length; counter++)
            if (board[counter][board.length - 1 - counter] != player)
                return false;
        return true;
    }

    public TTT_Constants checkWin_LargeBoard(TTT_Constants playerToCheck) {

        boolean colWin = false;
        for (int col = 0; col < board.length && !colWin; col++) {
            colWin = checkCol(col, playerToCheck);
        }

        boolean rowWin = false;
        for (int row = 0; row < board.length && !rowWin; row++) {
            rowWin = checkRow(row, playerToCheck);
        }
        if (checkMinorDiagonal(playerToCheck) || checkMajorDiagonal(playerToCheck) || rowWin || colWin) {
    ...
    //Only valid for 3x3 board
    public TTT_Constants checkWin() {
        final String win1String = TTT_Constants.PLAYER1.tag + TTT_Constants.PLAYER1.tag + TTT_Constants.PLAYER1.tag;
        final String win2String = TTT_Constants.PLAYER2.tag + TTT_Constants.PLAYER2.tag + TTT_Constants.PLAYER2.tag;
